...
This bot acts as a proactive site reliability engineer (SRE). It continuously monitors the production environment, gathers performance metrics, and uses the native DeepSeek-Reasoner (R1) model to generate intelligent, human-readable health reports directly in Telegram.
...
- **AI-Powered Diagnostics**: Integrates with the native DeepSeek API to analyze raw network metrics and translate them into actionable debugging advice for developers.
...
- \`DEEPSEEK_API_KEY\`: Your DeepSeek API key for LLM analysis.
